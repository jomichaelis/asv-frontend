...
def create_square_image(data):
    ...
    font_teams = ImageFont.truetype(io.BytesIO(font_bebas_neue), 155)
    font_liga = ImageFont.truetype(io.BytesIO(font_bebas_neue), 120)

    font_readex_pro_semibold = bucket.get_blob("fonts/ReadexPro-SemiBold.ttf").download_as_string()
    font_date = ImageFont.truetype(io.BytesIO(font_readex_pro_semibold), 80)
    ...
    draw.text((1000-(home_team_width1/2), 1140), data["homeName"], (255, 251, 0), font=font_teams)
    draw.text((1000-(minus_width1/2), 1260), minus, (255, 251, 0), font=font_teams)
    draw.text((1000-(guest_team_width1/2), 1380), data["guestName"], (255, 251, 0), font=font_teams)
    ...
    draw.text((1000-(date_a_width/2), 1650), data['date'], (255, 255, 255), font=font_date)
    draw.text((1000-(date_b_width/2), 1780), data['time'], (255, 255, 255), font=font_date)

    # Liga
    liga_width = draw.textlength(data['liga'], font=font_liga)
    draw.text((1000-(liga_width/2)+3, 152), data['liga'], (0, 0, 0), font=font_liga)
    draw.text((1000-(liga_width/2), 150), data['liga'], (255, 251, 0), font=font_liga)
